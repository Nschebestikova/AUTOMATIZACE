...
*** Test Cases ***
Login spatny email
    Login                      chyba           ${USER1_PASSWORD}
    Uzivatel neni prihlasen    ${ERROR_TEXT_FillCorrectEmail}

Login spatne heslo
    Login                      ${USER1_NAME}   bad
    Uzivatel neni prihlasen    ${ERROR_TEXT_IncorrectEmailOrPwd}

Login vse OK
    Login                   ${USER1_NAME}   ${USER1_PASSWORD}
    Uzivatel je prihlasen                   ${USER1_SHORT}
...
Uzivatel je prihlasen
    ...
    Get Text            ${SEL_HeaderLogin}               contains                ${Text}
...
Pred_sadou
    ...
    New Browser        headless=false     #dá se použít pro nastavení dalších parametru - umožnuje např vypnout headless mode

#    Open Browser       ${URL}                              headless=false     #dá se použít pro nastavení dalších parametru - umožnuje např vypnout headless mode
...
#    New Context    &{device}        # unpacking here with &
    ...
    New Context  recordVideo={'dir':'${OUTPUT_DIR}/video', 'size':{'width':1280, 'height':720}}  permissions=${permissions}   # záznam, rozlišení, povolení zjištování polohy
