...
Registrace na kurz
    [Arguments]  ${kurz}  ${name}  ${surname}  ${email}  ${tel}  ${osoba}    ${adresa}  ${ico}  ${pocet}  ${comment}  ${souhlas}
    ${json}=  ${json}=      Catenate  {"targetid":"","kurz":"${kurz}","name":"${name}","surname":"${surname}","email":"${email}","phone":"${tel}","person":"${osoba}","address":"${adresa}","ico":"${ico}","count":"${pocet}","comment":"${comment}","souhlas":${souhlas}}
    ${json_utf8}=  Encode String To Bytes    ${json}    UTF-8
    ${header}=  Create Directory  Content-Type=application/json
    ${resp}=  POST  #??  #odeslani zpravy a ulozeni do ${resp}
    Log  Responce: @{resp}
