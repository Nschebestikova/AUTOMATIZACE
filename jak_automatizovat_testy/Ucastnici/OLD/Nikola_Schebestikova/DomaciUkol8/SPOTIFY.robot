...
# můžeme si ještě PROJÍZ VYSVĚTLENÍ K TOMUTO PROSÍN???
    Log    ${base64encoded_client_id_and_secret}    level=info   formatter=repr
    #   Připravit header s
    ...
    Log     ${body}     level=debug         formatter=repr
    Log     ${body}      level=info         formatter=repr

    ${resp} =           POST  ${OAUTH_TOKEN_URL}  headers=${headers}  data=${body}  expected_status=200
     Log    ${resp}      level=debug         level=debug
...
#    • BONUS cvičení: Spotify (nepovinné)
#        ◦ Vytvořte API testy z pohledu funkcionality pro Spotify
#            ▪ stačí jen pár klíčových slov, např pro
#                1. Získání informací o umělci
#                2. TOP 10 songs konkrétního umělce
