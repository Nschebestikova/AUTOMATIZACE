...
Library         Browser
Library         DebugLibrary     # knihova pro ladeni, pokud chcete ledit test staci do prislusneho mista dat KS: Debug
Library         String
...
#Test Timeout    ${TC_TIMEOUT_ROBOT_KW}      # Timeout pro vsechny KS z RobotFW
#mělo by byt zakomentovano, protoze pokud delam test vice nez 2 minuty, neprojde to
...
Uklid_po_testu
    ...
    #pridat odhlaseni pokud je uzivatel prihlasen
    ${menu} =   Get Text       ${SEL_NavigationMenu}
    Log to Console   >>>${menu}<<<
    #Debug
    ${clean_menu} =    Replace String    ${menu}   ${SEL_LoginLink}   ${\n}  #\n    " "    # Odstranuje nove radky
    Log to Console  ${clean_menu}
    IF  "Odhlásit se" in """${clean_menu}"""
