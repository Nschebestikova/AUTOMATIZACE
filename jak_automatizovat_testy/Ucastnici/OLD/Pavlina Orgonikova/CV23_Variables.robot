...
Ukázka práce s proměnnu typu List
    #Seznamy se hodí např. na ukládání údajů z webu (uložení seznamu prvků)

    @{list1} =      Create List    first    second    third
    Log to console  ${list1}[1]
    Log to Console  ${list}[2]

    # Vytvoření seznamů jako v Pythonu (díky KS Evaluate)
    ${list2} =      Evaluate    ['a', 'b', 'c']

    ${list3} =      Evaluate    [['a', 'b', 'c'], {'key': ['x', 'y']}]
    Log Many        @{list3}[0]         # Logs 'a', 'b' and 'c'.
    Log Many        @{list3}[1][key]    # Logs 'x' and 'y'.
    Log Many        @{list3}[1]         # Logs 'key'

    #převedení seznamu na scalar
    Log To Console    >>>${list1}<<<
    # Pozor tohle může někde fungovat ale né vždy
    Log To Console    >>>@{list1}<<<
    # Správné použiti je
    Log Many  @{list1}
...
Ukázka práce s proměnnou typu Environment
    Log to console  ${\n}>>> Username %{USERNAME}
