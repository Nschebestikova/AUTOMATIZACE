...
#KW compares data of ice rinks from template and actual file
#firstly the keys from dictionaries are compared (they are saved in list and the both lists are compared)
#secondly the values from dictionaries are compared via key from template file
...
Compare load time
#KW checks the difference of load times from template file and actual file and compares it with allowed difference from configuration file
#difference (in seconds) = load_time_actual - load_time_template
#allowed amount of difference time ${FILE_time_difference} is set in configuration file
    ...
    #compare load times -> difference (load_time_template - load_time_actual)
    ${time_difference} =    Evaluate    ${load_time_actual} - ${load_time_template}
    Log    ${time_difference}
...
#KW compares amount of ice rinks in template file and actual file
...
#KW creates dictionaries of ice rinks data from page and saves them to file,
#path of file ${FILE_path} and extenstion of file ${FILE_extension} are set in configuration file
#every dictionary is saved in new line of file
#first dictionary contains information about page, key = page_info, value = {title_of_page, load_time}
#example of information page dictionary: 'page_info': {'title_of_page': 'Veřejné bruslení ČR', 'load_time': '0:00:04.665524'}
#ice rink's dictionary: key = title of ice rink, value = {title of page, url of photo, status if ice rink is opened or closed}
#example of ice rink: 'TÁBOR | ZIMNÍ STADION': {'title': 'TÁBOR | ZIMNÍ STADION', 'photo': '/wp-content/uploads/2018/01/ilustracni-foto-kluziste.jpg', 'status': 'Zavřeno'}
...
Read from file
#KW takes the file and saves the data of file to variable ${content_dict}
...
#KW checks the load time of page and saves it to variable ${result}
...
Generate name of file
#KW generates the name of file
#format of name: year_month_day_hour_minutes_url
#name is saved to variable ${file_name}
...
#KW saves the ice rinks names to list
...
Consent dialog
#KW clicks to button Consent in Consent dialog
#consent dialog is displayed for the first time of opening page
